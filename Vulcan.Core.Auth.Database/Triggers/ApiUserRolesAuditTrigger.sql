...
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE [type] = 'TR' AND [name] = '[$(NewSchema)].[ApiUserRolesAuditTrigger]'
    )
DROP TRIGGER [$(NewSchema)].[ApiUserRolesAuditTrigger];

GO

CREATE TRIGGER [$(NewSchema)].[ApiUserRolesAuditTrigger]
	ON [$(NewSchema)].[ApiUserRoles]
	after DELETE, INSERT, UPDATE
	AS
	BEGIN
		declare @TableName varchar(64)
		set @TableName = '[auth].[ApiUserRoles]'
