...
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE [type] = 'TR' AND [name] = '[$(NewSchema)].[ApiClientRolesAuditTrigger]'
    )
DROP TRIGGER [$(NewSchema)].[ApiClientRolesAuditTrigger];

GO

CREATE TRIGGER [$(NewSchema)].[ApiClientRolesAuditTrigger]
	ON [$(NewSchema)].[ApiClientRoles]
	after DELETE, INSERT, UPDATE
	AS
	BEGIN
		declare @TableName varchar(64)
		set @TableName = '[auth].[ApiClientRoles]'
